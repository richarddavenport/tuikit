...
//
// The deploy tool's renderdeploy.go (809 lines), the database tool's
// actionrun.go (370) and the cloud tool's runner.go (262) each have one, and
// democtl's is a fourth. All four agree on the shape: a glyph per state, the
// step's name, and something trailing that says what it cost or why it did not
// run.
//
// The meaningful difference is WHERE that trailing thing goes. The cloud tool
// puts it inline after the label; democtl right-aligns it against the pane's
// edge. It turns out they are not the same thing badly agreed on — they are
// two things. A duration is a number you scan down a column, so it goes right.
// A skip reason or a dry-run command is prose about that step, so it sits
// beside it. Both, and no flag.
//
...
// nothing about steps. The cloud tool's playbooks are longer than its pane, so
// it needs both, and it was writing this adapter by hand — twenty lines of
// mechanism living in a tool because the component could not do the job.
//
...
// click on it to select the step it belongs to, has a real requirement — and
// it is not this function's to guess at.
