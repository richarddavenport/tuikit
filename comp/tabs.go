...
//
// The deploy tool, the database tool and democtl each have one. They
...
// outright. So the chevrons go around the strip.
//
// The count. The deploy tool alone can write Pending(3), for a tab with things
// behind it. That is a real requirement rather than an accident, and cheap: a
// tab with nothing pending simply has no count.
//
// The database tool's third difference — active-when-focused styled
// differently from active-when-not — is kept, from the deploy tool, which does
// the same. A tab strip that looks identical whether or not you can press the
// keys is a strip that invites you to press them.
type Tabs struct {
	Tabs   []Tab
	Active int
...
// Tab is one entry. A Count above zero is drawn beside the name, for a tab
// with things waiting behind it.
type Tab struct {
	Name  string
	Count int
}
