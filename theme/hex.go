...
//
// It answers "" for a color it cannot read, and NOT "#000000": index 0 is a
...
//
// Computed rather than tabled, because the 256-color palette IS a formula for
// everything above index 15: a 6×6×6 cube, then a 24-step gray ramp. A table
...
// captured for; recoloring it would report a tool that does not exist. A
...
func Hex(c lipgloss.TerminalColor) string {
	...
	default:
		// The gray ramp: 24 steps from 8 to 238, ten apart.
		g := 8 + 10*(i-232)
		return rgb(g, g, g)
	}
}

// cubeLevel is the six values a channel takes in the color cube. Not evenly
// spaced: the gap from nothing to the first step is larger, so dark colors
...
// normalize expands the three-digit form, so #fff and #ffffff mean the same
...
// Value is a role's color as the tool DECLARED it — "13", "#d2a8ff", or a
// light/dark pair.
//
...
// from 16 up is a formula — a 6x6x6 cube and a gray ramp — that no theme
