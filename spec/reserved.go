...
// # Why impose anything
//
// Everything else here is a part you take or leave. This is a rule, because
// the value is entirely in it being the same everywhere: a reader who has used
// one of these tools presses `q` expecting to leave, and a tool where `q`
// means "queue" has set a trap using the other tools' credibility. One tool
// can be idiosyncratic; a family cannot.
//
...
// immediately, and both are right). It may not make `q` mean something that is
// not leaving.
//
...
//
// Matched on the name rather than a flag on Command, because a flag is a
// second thing to keep in step with the name and this is not a close call in
// practice: a command bound to `q` is called quit, exit, back, close or
// cancel, or it is the bug this catches.
